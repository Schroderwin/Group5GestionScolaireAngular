...
export class ScheduleComponent  {
  ...
  handleSelect(selectInfo: DateSelectArg) {
    ...
    this.startDay = new Date(selectInfo.startStr).toISOString().split('T')[0]
    //this.startTime = new Date(selectInfo.start).getHours();
    this.startTime = new Date(selectInfo.start).toLocaleTimeString();
    ...
    if (this.ourForm.valid) {
      ...
      let scheduleEvent:ScheduleEvent = this.ourForm.value as ScheduleEvent
      scheduleEvent.groupClass = this.groupClass

      /*this.router.navigateByUrl(`/institution/1/group-class/1/schedule`);*/
      this.modalService.dismissAll();
      console.log(this.ourForm.value)

      this.ses.add(scheduleEvent).subscribe(t => this.router.navigateByUrl(`/group-class/${this.groupClass.id}/schedule`))


      let events = calendarApi.addEvent({
        id: createEventId(),
